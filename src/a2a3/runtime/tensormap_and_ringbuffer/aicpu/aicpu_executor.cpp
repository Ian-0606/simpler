...
struct AicpuExecutor {
    ...
    // Simpler AICPU affinity policy (used by affinity tests).
 ...
 */
 void AicpuExecutor::reassign_cores_for_all_threads() {
    ...
    // 收集所有线程上当前正在运行/空闲的 core 状态，用于后面恢复执行中的任务
    ...
    for (int32_t t = 0; t < thread_num_; ++t) {
        // AIC running
        for (int32_t j = 0; j < trackers_[t].aic().running_count; ++j) {
            int32_t core_id = trackers_[t].aic().running[j];
            running_cores[running_count] = core_id;
            running_task_ids[running_count] = executing_task_ids_[t][core_id];
            ++running_count;
        }
        // AIC idle
        for (int32_t j = 0; j < trackers_[t].aic().idle_count; ++j) {
            was_idle[trackers_[t].aic().idle[j]] = true;
        }

        // AIV running
        for (int32_t j = 0; j < trackers_[t].aiv().running_count; ++j) {
            int32_t core_id = trackers_[t].aiv().running[j];
            running_cores[running_count] = core_id;
            running_task_ids[running_count] = executing_task_ids_[t][core_id];
            ++running_count;
        }
        // AIV idle
        for (int32_t j = 0; j < trackers_[t].aiv().idle_count; ++j) {
            was_idle[trackers_[t].aiv().idle[j]] = true;
        }
    }

    // 清空所有线程的 tracker 与执行状态
    for (int32_t t = 0; t < thread_num_; ++t) {
        core_count_per_thread_[t] = 0;
        trackers_[t].aic().running_count = 0;
        trackers_[t].aic().idle_count = 0;
        trackers_[t].aiv().running_count = 0;
        trackers_[t].aiv().idle_count = 0;
        trackers_[t].cluster_count = 0;
        memset(trackers_[t].core_idle, 0, sizeof(trackers_[t].core_idle));
        for (int32_t j = 0; j < MAX_CORES_PER_THREAD; ++j) {
            executing_task_ids_[t][j] = AICPU_TASK_INVALID;
        }
    }

    // 将单个 worker_id 的 core 重新挂到某个线程的 tracker 上，并尽量保持原来运行中的任务
    auto reassign_core = [&](int32_t worker_id,
                             CoreTypeTracker& type_tracker,
                             CoreStateTracker& tracker,
                             int32_t thread_idx) {
        core_assignments_[thread_idx][core_count_per_thread_[thread_idx]++] = worker_id;

        // 如果这个 core 之前在 running 列表里，就恢复它的执行状态
    ...
    // 以“cluster”为单位，把 AIC + 对应的 2 个 AIV 一组，轮询分配到各线程
    ...
    // 打印最终分布（包含 role，方便你看 affinity 逻辑）
    DEV_INFO("Core reassignment complete:");
    for (int32_t t = 0; t < thread_num_; ++t) {
        DEV_INFO("  Thread %d: %d cores, %d clusters (AIC: running=%d idle=%d, AIV: running=%d idle=%d, role=%d)",
                 t,
